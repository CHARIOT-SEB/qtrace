...
export function SecTrace({
	...
	useEffect(() => {
		setLocalBuffer(bufferRange)
	}, [bufferRange])
	useEffect(() => {
		setLocalSignal(signalRange)
	}, [signalRange])
	...
	const DotShape = (props: any) => {
		...
		return (
			<circle cx={cx} cy={cy} r={4} fill={payload.color} fillOpacity={0.9} />
		)
	}
